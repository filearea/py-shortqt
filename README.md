...
如果服务器需要通过代理访问币安 API，**必须在启动前**配置 `config/runtime.json` 中的代理参数：
...
> **关键约束：**
> - 代理配置在**启动时**加载并写入系统环境变量，API 连接和 WebSocket 行情流在初始化时即绑定代理
> - **启动后修改代理配置不会自动生效**，需要重启程序（或在 TUI 设置面板修改后触发重连）
> - 代理未启用时，程序会自动清除 `HTTP_PROXY` / `HTTPS_PROXY` 环境变量，避免残留值干扰直连
> - 如遇 API 连接超时或 WebSocket 无法建立，请首先检查代理是否正确配置

### Web 访问注意事项

Web 看板通过 **token 认证**，仅限内网访问：

- **Token 机制**：`web_ui.token` 配置为空时，启动时自动随机生成；有值时固定使用配置值（持久化，重启不变）
- 启动后在 TUI 或终端日志中查找完整访问地址：`http://<内网IP>:8099?token=<token>`
- **不带 token 或 token 错误会返回 403 页面**，无法访问任何功能
- 手机和 PC 必须在**同一局域网**内（连接同一个路由器/Wi-Fi）
- **安全警告**：如使用固定 token，请自行保证安全性。**强烈不建议开放外网端口转发**，如需远程访问应使用 VPN 或 SSH 隧道
- 暂不支持多个客户端同时连接（单 WebSocket 连接设计）
...
见上方 [⚠️ 启动前必读 — 代理配置](#️-启动前必读)，在 `config/runtime.json` 中设置 `proxy.enabled` 等字段。
