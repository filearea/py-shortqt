# py-shortqt v1.4.0 - Maker Scalper 剥头皮交易系统

> 基于币安 Futures API 的 ETHUSDC 合约自动交易系统
> **支持实盘模式/模拟模式 | 完整日志系统 | TUI 设置模块 | 动态指标系统 | 实时数据记录**
...
- **当前版本：** v1.4.0
- **发布日期：** 2026-03-28
- **核心功能：** 实盘交易 + 模拟交易 + TUI 设置模块 + 完整日志系统 + 动态指标系统 + 实时数据记录
...
实时计算 8 个关键市场指标，提供综合评分（0-100 分）+ 交易建议（适合/观望/暂停）。
...
| 类别 | 指标 | 计算方式 | 预警阈值 |
|------|------|---------|---------|
| **波动率** | 1 分钟振幅 | (最高 - 最低) / 最新价 × 100% | < 0.03% 🟥 / > 0.3% 🟩 |
| | 5 分钟振幅 | 近 5 根 K 线综合振幅 | - |
| | 1 小时平均振幅 | 近 60 根 K 线平均振幅 | < 0.08% 🟥 |
| | 变化率 | 当前价相对 1 小时前变化 | < 0.5% 🟥 / > 2% 🟩 |
| | ATR(14) | 14 周期平均真实波幅 | - |
| **流动性** | 买卖价差 | ask - bid | < 0.01 USDC 🟩 |
| | 价差率 | 价差 / 中间价 × 100% | < 0.05% 🟩 / > 0.2% 🟥 |
| | 市场深度 | 前 10 档挂单总量 | > 50 ETH 🟩 |
| | 深度不平衡 | (买量 - 卖量) / 总量 | -0.2 ~ 0.2 🟩 |

**综合评分：**
- 🟢 **适合** (60-100 分) - 波动率和流动性良好，适合剥头皮
- 🟡 **观望** (30-59 分) - 部分指标不达理想，谨慎操作
- 🔴 **暂停** (0-29 分) - 市场条件不佳，暂停交易

**数据记录：**
- 每 30 秒自动保存指标数据到 `data/metrics/`
- JSONL 格式，便于后续分析回测
...
**v1.4.1 新增** - 自动记录实时 K 线和订单簿数据到 `data/` 目录。
...
- ✅ **启动时记录** - 软件启动后记录初始余额（`event: startup`）
- ✅ **平仓后记录** - 每次彻底平仓后记录余额（`event: position_closed`）
- ✅ **关闭前记录** - 用户关闭系统前记录最终余额（`event: shutdown`）
- ✅ **7 种平仓场景** - TP/SL/STOP_MARKET/MANUAL/SL_FALLBACK/sync_detected/MARKET_CLOSE

**日志格式：**
```json
{"timestamp":"2026-03-25 13:13:32.059","type":"BALANCE","event":"startup","balance":3.50845185,"details":{"account":"储蓄账户","leverage":5}}
{"timestamp":"2026-03-25 13:14:00.123","type":"BALANCE","event":"position_closed","balance":3.55845185,"details":{"reason":"TP","pnl":0.05}}
```

**使用方法：** 交易部门可读取 `logs/<run_id>/trades.log` 计算单次收益率、复合收益率、最大回撤等指标。

**已知问题：** Windows 关闭窗口时 shutdown 日志可能无法写入，建议按 Q 键退出。
...
- ✅ **系统日志** - 程序运行状态、错误追踪
- ✅ **市场日志** - 订单簿快照、价格变动、信号触发
- ✅ **交易日志** - 订单、持仓、平仓记录（JSONL 格式）
- ✅ **信号 CSV** - 自动记录入场特征和平仓结果
- ✅ **日志查看工具** - 支持过滤、搜索、统计
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.3.3 | 2026-03-25 | 账户余额日志（用于复合收益率计算） |
| v1.3.2 | 2026-03-24 | 修复止损单 PnL 记录缺失 Bug |
| v1.3.1 | 2026-03-23 | 修复保底止损计算错误 |
| v1.3.0 | 2026-03-23 | 完整日志系统、保底止损优化 |
